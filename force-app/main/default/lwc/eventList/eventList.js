...
import dtatTable from "@salesforce/apex/eventListrec.dtatTable";
const columns = [
  {
    label: "View",
...
export default class EventList extends LightningElement {
  ...
  handleSearch(event) {
    ...
    let filteredEvents = this.result.filter((record) => {
  ...
  handleStartDate(event) {
    ...
    let filteredEvents = this.result.filter((record) => {
      return record.Startingdate__c >= valuedatetime;
    });
    this.recordsToDisplay = filteredEvents;
  }
  handleLocationSearch(event) {
    ...
    let filteredEvents = this.result.filter((record) => {
      return record.Location_project__c.toLowerCase().includes(keyword.toLowerCase());
    });
    if (keyword && keyword.length >= 2) {
      this.recordsToDisplay = filteredEvents;
    } else {
